...
fn build_cli() -> App<'static, 'static> {
    App::new("esptool")
        ...
        .subcommand(SubCommand::with_name("load_ram").about(
            "Download an image to RAM and execute",
        ))
        .subcommand(SubCommand::with_name("dump_mem").about(
            "Dump arbitrary memory to disk",
        ))
        .subcommand(SubCommand::with_name("read_mem").about(
            "Read arbitrary memory location",
        ))
        .subcommand(SubCommand::with_name("write_mem").about(
            "Read-modify-write to arbitrary memory location",
        ))
        .subcommand(SubCommand::with_name("write_flash").about(
            "Write a binary blob to flash",
        ))
        .subcommand(SubCommand::with_name("run").about(
            "Run application code in flash",
        ))
        .subcommand(SubCommand::with_name("image_info").about(
            "Dump headers from an application image",
        ))
        .subcommand(SubCommand::with_name("make_image").about(
            "Create an application image from binary files",
        ))
        .subcommand(SubCommand::with_name("elf2image").about(
            "Create an application image from ELF file",
        ))
        .subcommand(SubCommand::with_name("read_mac").about(
            "Read MAC address from OTP ROM",
        ))
        .subcommand(SubCommand::with_name("chip_id").about(
            "Read Chip ID from OTP ROM",
        ))
        .subcommand(SubCommand::with_name("flash_id").about(
            "Read SPI flash manufacturer and device ID",
        ))
        .subcommand(SubCommand::with_name("read_flash_status").about(
            "Read SPI flash status register",
        ))
        .subcommand(SubCommand::with_name("write_flash_status").about(
            "Write SPI flash status register",
        ))
        .subcommand(SubCommand::with_name("read_flash").about(
            "Read SPI flash content",
        ))
        .subcommand(SubCommand::with_name("verify_flash").about(
            "Verify a binary blob against flash",
        ))
        .subcommand(SubCommand::with_name("erase_flash").about(
            "Perform Chip Erase on SPI flash",
        ))
        .subcommand(SubCommand::with_name("erase_region").about(
...
fn main() {
    ...
    let baud_rate = value_t!(args.value_of("BAUD"), usize).unwrap_or_else(|e| match e.kind {
